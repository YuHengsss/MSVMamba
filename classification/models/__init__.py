...
def build_vssm_model(config, is_pretrain=False):
    ...
    if model_type in ["vssm"]:
        model = VSSM(
            ...
            pre_norm=config.MODEL.VSSM.PRE_NORM,
            add_se = config.MODEL.VSSM.ADD_SE,
            convffn=config.MODEL.VSSM.CONVFFN,
            axis_stage = config.MODEL.VSSM.AXIS_STAGE,
            num_heads = config.MODEL.VSSM.NUM_HEADS,
            conv_ffn_ratio = config.MODEL.VSSM.CONV_FFN_RATIO,
            full_res_index = config.MODEL.VSSM.FULL_RES_INDEX,
        )
        return model
    return None
