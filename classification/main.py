...
def parse_option():
    parser = argparse.ArgumentParser('MSVMamamba training and evaluation script', add_help=False)
...
def main(config):
    ...
    if config.MODEL.RESUME:
        max_accuracy, max_accuracy_ema = load_checkpoint_ema(config, model_without_ddp, optimizer, lr_scheduler,
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        if epoch / config.TRAIN.EPOCHS < 0.8:  # TODO: for temporary use
            if dist.get_rank() == 0:
