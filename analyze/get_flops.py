...
if __name__ == '__main__':
    ...
    if True:
        ...
        if True:
            mmdet_flops(config=f"{detpath}/ms_vssm/mask_rcnn_ms_vssm_fpn_coco_nano.py") #26.569M 187540485448.0
            mmdet_flops(config=f"{detpath}/ms_vssm/mask_rcnn_ms_vssm_fpn_coco_micro.py") # 31.518M 201267932592.0
            mmdet_flops(config=f"{detpath}/ms_vssm/mask_rcnn_ms_vssm_fpn_coco_tiny.py") # 52.553M 264953687138.56

            mmseg_flops(config=f"{segpath}/ms_vssm/upernet_ms_vssm_4xb4-160k_ade20k-512x512_tiny.py", input_shape=(3, 512, 2048)) # GFlops:  942.51879152 Params:  64699820
            mmseg_flops(config=f"{segpath}/ms_vssm/upernet_ms_vssm_4xb4-160k_ade20k-512x512_micro.py", input_shape=(3, 512, 2048)) # GFlops:  875.113123936 Params:  41674796
            mmseg_flops(config=f"{segpath}/ms_vssm/upernet_ms_vssm_4xb4-160k_ade20k-512x512_nano.py", input_shape=(3, 512, 2048)) # GFlops:  860.014528744 Params:  35730668